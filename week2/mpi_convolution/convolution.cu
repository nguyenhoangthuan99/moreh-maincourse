...
static int ngpu;
static float *I_gpu[1024], *F_gpu[1024], *BUF1_gpu[1024];
...
void convolution_thread(float *_I, float *_F, float *_O, float *_BUF1, float *_BUF2, int N,
                        ...
                        int stride_h, int stride_w, int dilation_h, int dilation_w, int gpu_id = 0)
{
  ...
  cudaMemcpyAsync(F_gpu[gpu_id], _F, sizeof(float) * K * C * R * S, cudaMemcpyHostToDevice,data_h2d_stream); // data_h2d_stream

  for (int i = 0; i < num_stream_blocks; i++)
  {
    cudaMemcpyAsync(&I_gpu[gpu_id][Nbegin[i] * C * H * W], &_I[Nbegin[i] * C * H * W], sizeof(float) * (Nend[i] - Nbegin[i]) * C * H * W, cudaMemcpyHostToDevice, streams[i]);
...
void convolution_node(float *_I, float *_F, float *_O, float *_BUF1, float *_BUF2, int N,
                      int C, int H, int W, int K, int R, int S, int pad_h, int pad_w,
                 ...
                 int stride_h, int stride_w, int dilation_h, int dilation_w, int mpi_rank, int mpi_world_size)
{
  ...
  MPI_Ibcast(_F, K * C * R * S, MPI_FLOAT, 0, MPI_COMM_WORLD, request);

  MPI_Iscatterv(_I, sendcounts, displ, MPI_FLOAT, _I, N * C * H * W, MPI_FLOAT, 0, MPI_COMM_WORLD, request + 1);
                            ...
                            int dilation_h, int dilation_w)
{
  ...
  {
    cudaSetDevice(i);
    CHECK_CUDA(cudaMalloc(&I_gpu[i], sizeof(float) * (N_gpu_end[i] - N_gpu_start[i]) * C * H * W));
    // CHECK_CUDA(cudaMalloc(&O_gpu, sizeof(float) * ON * OH * OW * OC));
    CHECK_CUDA(cudaMalloc(&F_gpu[i], sizeof(float) * K * C * R * S));
    CHECK_CUDA(cudaMalloc(&BUF1_gpu[i], sizeof(float) * C * R * S * (N_gpu_end[i] - N_gpu_start[i]) * OH * OW));
