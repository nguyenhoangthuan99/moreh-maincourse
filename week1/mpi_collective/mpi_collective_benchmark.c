...
{
  // TODO: FILL_IN_HERE
  CHECK_MPI(MPI_Allreduce(sendbuf, recvbuf, buf_size_mb * MB, MPI_BYTE, MPI_SUM, MPI_COMM_WORLD));
}

static void test_Bcast(void)
{
  // TODO: FILL_IN_HERE
  CHECK_MPI(MPI_Bcast(sendbuf, buf_size_mb * MB, MPI_BYTE, root, MPI_COMM_WORLD));
}
...
{
  // TODO: FILL_IN_HERE
  CHECK_MPI(MPI_Gather(sendbuf, buf_size_mb * MB, MPI_BYTE, recvbuf, buf_size_mb * MB, MPI_BYTE, root, MPI_COMM_WORLD));
}

static void test_Allgather(void)
{
  // TODO: FILL_IN_HERE
  CHECK_MPI(MPI_Allgather(sendbuf, buf_size_mb * MB, MPI_BYTE, recvbuf, buf_size_mb * MB, MPI_BYTE, MPI_COMM_WORLD));
}

static void test_Alltoall(void)
{
  // TODO: FILL_IN_HERE
  CHECK_MPI(MPI_Alltoall(sendbuf, buf_size_mb * MB / mpi_world_size, MPI_BYTE, recvbuf, buf_size_mb * MB / mpi_world_size, MPI_BYTE, MPI_COMM_WORLD));
}

static void test_Reduce_scatter(void)
{
  // TODO: FILL_IN_HERE
  int *revcount = (int*) malloc(mpi_world_size*sizeof(int)) ;
  for(int i=0;i<mpi_world_size;i++){
    revcount[i] = (buf_size_mb * MB) / mpi_world_size;
  }
  // const int b[mpi_world_size] = a;
  CHECK_MPI(MPI_Reduce_scatter(sendbuf, recvbuf, revcount, MPI_BYTE, MPI_SUM, MPI_COMM_WORLD));
}

static void test_Scan(void)
{
  // TODO: FILL_IN_HERE
  CHECK_MPI(MPI_Scan(sendbuf, recvbuf, buf_size_mb * MB, MPI_BYTE, MPI_SUM, MPI_COMM_WORLD));
}
...
int main(int argc, char **argv)
{
  ...
  sendbuf = (int *)malloc(buf_size_mb * MB);
  recvbuf = (int *)malloc(buf_size_mb * MB);
